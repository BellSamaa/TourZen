...
// LUÔN DÙNG FILE 'tours.js' DUY NHẤT
...
const TourDetail = () => {
  ...
  useEffect(() => {
    // Tự động chọn tháng đầu tiên khi tải trang
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto p-4 -mt-32 relative z-10">
        <motion.section className="max-w-5xl mx-auto" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            {galleryImages.map((src, i) => (
              <div key={i} className="px-2">
                <img
                  ...
                  className="rounded-xl shadow-lg h-[300px] md:h-[500px] object-cover w-full"
                />
              </div>
            ))}
          </Slider>
        ...
        <motion.section className="max-w-6xl mx-auto p-4 md:p-8 bg-white rounded-2xl shadow-lg mt-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-3xl font-bold mb-6 text-center text-blue-800">LỊCH KHỞI HÀNH</h2>
          {tour.departureMonths?.length > 0 ? (
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex overflow-x-auto md:overflow-x-visible md:flex-col gap-3 md:w-1/4 pb-2 md:pb-0">
                {tour.departureMonths.map((monthData) => (
                  <button
                    key={monthData.month}
                    onClick={() => setActiveMonthData(monthData)}
                    className={`p-4 rounded-lg font-semibold border-2 w-full text-left transition duration-300 ease-in-out transform hover:-translate-y-1 whitespace-nowrap ${
              ...
              {activeMonthData && (
                <div className="flex-1 bg-gray-50 p-6 rounded-xl shadow-inner">
                  ...
                  <div className="space-y-4 text-sm">
                    <div className="flex items-start"><FaGift className="text-orange-500 text-lg mr-3 mt-1 flex-shrink-0" /><p><span className="font-semibold">Ưu đãi tháng:</span> {activeMonthData.promotions}</p></div>
                    <div className="flex items-start"><MdFamilyRestroom className="text-green-500 text-lg mr-3 mt-1 flex-shrink-0" /><p><span className="font-semibold">Phù hợp cho:</span> {activeMonthData.familySuitability}</p></div>
                    <div className="flex items-start"><FaPlaneDeparture className="text-sky-500 text-lg mr-3 mt-1 flex-shrink-0" /><p><span className="font-semibold">Thông tin chuyến bay:</span> {activeMonthData.flightDeals}</p></div>
                  </div>
        ...
        <motion.section className="max-w-6xl mx-auto p-6 mt-8 bg-white rounded-2xl shadow-lg" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              tour.itinerary.map((item, i) => (
                <div key={i} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className="bg-blue-600 text-white rounded-full w-10 h-10 flex items-center justify-center font-bold flex-shrink-0">{i + 1}</div>
        ...
        <motion.section className="max-w-5xl mx-auto my-10" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-3xl font-bold mb-6 text-center text-blue-800">Vị Trí Điểm Đến</h2>
