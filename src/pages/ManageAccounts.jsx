...
// <<< UPGRADE: Định nghĩa variants cho hiệu ứng stagger trong Modal
const modalFormVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        delay: 0.1, // Chờ modal mở ra
        staggerChildren: 0.07 // Hiệu ứng cho từng field
      }
    }
};
const fieldVariant = {
...
const AccountModal = ({ account, onClose, onSuccess }) => {
    ...
    const [showPassword, setShowPassword] = useState(false); // <<< UPGRADE: State cho show/hide pass
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                ...
                if (profileError) {
                    throw new Error(`Tạo profile thất bại: ${profileError.message}.`);
                }
    ...
    return (
        ...
        >
            <motion.div
                ...
                transition={{ duration: 0.2, type: "spring", stiffness: 200, damping: 25 }} // <<< UPGRADE: Hiệu ứng spring
            >
                <div className="flex justify-between items-center p-4 border-b dark:border-slate-700">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-white">
                        {isEdit ? 'Chỉnh sửa Tài khoản' : 'Thêm Tài khoản mới'}
                    </h3>
                    {/* <<< UPGRADE: Thêm hiệu ứng cho nút X */}
                    <motion.button 
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                    ...
                    >
                        ...
                        {!isEdit && (
                            <motion.div variants={fieldVariant}>
                                ...
                                {/* <<< UPGRADE: Thêm wrapper cho icon password */}
                                <div className="relative">
                                    <input 
                                        type={showPassword ? "text" : "password"}
                                        name="password" value={formData.password} onChange={handleChange} 
                                        required minLength="6"
                                        className="input-style pr-10" // Thêm padding
                                        placeholder="Tối thiểu 6 ký tự"
                                    />
                                    <button 
                                        type="button" 
                                        onClick={() => setShowPassword(!showPassword)}
                                        className="password-toggle-btn" // Xem style ở dưới
                    ...
                    <div className="p-4 border-t dark:border-slate-700 flex justify-end gap-3 bg-slate-50 dark:bg-slate-800">
                        {/* <<< UPGRADE: Thêm hiệu ứng cho nút Hủy */}
                        <motion.button 
                        ...
                        {/* <<< UPGRADE: Thêm hiệu ứng cho nút Lưu */}
                        <motion.button 
...
// <<< UPGRADE: Định nghĩa variants cho hiệu ứng Stagger tải trang
const pageVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.1 // Mỗi item con cách nhau 0.1s
      }
    }
};
const itemVariant = {
...
export default function AdminManageAccounts() {
        ...
        toast((t) => (
            <div className="flex flex-col items-center p-1">
                ...
                <div className="mt-3 flex gap-2">
                 <motion.button // <<< UPGRADE: Thêm hiệu ứng
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            animate="visible"
            // Bỏ space-y-6 ở đây và đặt vào container con
        >
            {/* <<< UPGRADE: Container cho hiệu ứng Stagger */}
            <motion.div 
                className="space-y-6"
            ...
            >
                ...
                <motion.div variants={itemVariant} className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    {/* <<< UPGRADE: Thêm hiệu ứng cho nút Thêm */}
                    <motion.button 
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                ...
                {/* Bảng */}
                <motion.div
                    variants={itemVariant} // <<< UPGRADE: Stagger item
                    ...
                    <div className="p-4 flex flex-wrap justify-between items-center gap-3 border-b dark:border-slate-700">
                        ...
                        <div className="flex items-center gap-3 w-full sm:w-auto">
                            ...
                            <motion.button // <<< UPGRADE: Thêm hiệu ứng
                                ...
                                className={`p-2 button-secondary ${isFetchingPage ? 'opacity-50' : ''}`} title="Làm mới"
                            >
                    ...
                    <div className="overflow-x-auto relative">
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                <AnimatePresence> {/* <<< UPGRADE: Cho phép hiệu ứng exit khi xóa */}
                                    ...
                                    {!error && accounts.map((account) => (
                                        <motion.tr
                                            key={account.id}
                                            layout // <<< Cho phép di chuyển mượt
                                            initial={{ opacity: 0, y: -10 }} 
                                            animate={{ opacity: 1, y: 0 }} 
                                            exit={{ opacity: 0, x: -50, transition: { duration: 0.2 } }} // <<< Hiệu ứng khi xóa
                                        ...
                                        >
                                            ...
                                            <td className="td-style text-center space-x-2">
                                                {/* <<< UPGRADE: Thêm hiệu ứng cho nút Sửa */}
                                                <motion.button 
                ...
                {!loading && totalItems > ITEMS_PER_PAGE && (
                    ...
                    >
                        ...
                        <nav className="flex items-center gap-1 mt-3 sm:mt-0">
                            {/* <<< UPGRADE: Thêm hiệu ứng cho nút phân trang */}
            ...
            </motion.div> {/* Kết thúc container stagger */}
            ...
            <style jsx>{`
                ...
                .th-style { @apply px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap; }
                .td-style { @apply px-5 py-3 text-sm; }
                .td-center { @apply px-6 py-10 text-center; }
                .badge-base { @apply px-2.5 py-1 text-xs font-semibold rounded-md inline-flex items-center gap-1.5 whitespace-nowrap; } /* <<< UPGRADE: Tăng padding, gap */
                ...
                .button-primary { @apply bg-slate-800 hover:bg-slate-900 dark:bg-white dark:hover:bg-gray-200 text-white dark:text-slate-900 font-semibold px-4 py-2 rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed; }
                ...
                /* <<< UPGRADE: Style cho nút password toggle */
                .password-toggle-btn {
