...
const AccountModal = ({ account, onClose, onSuccess }) => {
        ...
        try {
            if (isEdit) {
                ...
                const { error: updateError } = await supabase
                    ...
                    .eq('id', account.id); // <<< SỬA TỪ 'uuid' SANG 'id'
                ...
                const { error: profileError } = await supabase
                    .from('Users')
                    .insert({
                        id: authData.user.id, // <<< SỬA TỪ 'uuid' SANG 'id'
                        email: formData.email,
                        username: formData.username,
                        full_name: formData.full_name,
                        role: formData.role,
    ...
    // ... (JSX của Modal giữ nguyên, không cần thay đổi) ...
    return (
        <motion.div
                        ...
                        {!isEdit && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                                ...
                                <input 
                                    ...
                                    className="input-style" placeholder="Tối thiểu 6 ký tự"
                                />
                            </motion.div>
                        )}
...
export default function AdminManageAccounts() {
    ...
    const fetchAccounts = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            // (FIXED) Query từ bảng 'Users' và select 'id'
            const selectQuery = `
                ...
                <div className="mt-3 flex gap-2">
                 <button
                    ...
                    onClick={async () => {
                        ...
                        // (FIXED) Cập nhật 'Users' và dùng 'id'
                        const { error: updateError } = await supabase
                            .from("Users")
    ...
    return (
        ...
        >
            ...
            >
                <div className="p-4 flex flex-wrap justify-between items-center gap-3 border-b dark:border-slate-700">
                     ...
                     <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <button onClick={() => fetchAccounts(false)} disabled={isFetchingPage} className={`p-2 button-secondary ${isFetchingPage ? 'opacity-50' : ''}`} title="Làm mới">
            ...
            <AnimatePresence>
                {modalAccount && (
                    <AccountModal
                        key={modalAccount.id || 'new'} // <<< SỬA KEY
            ...
            <style jsx>{`
                ...
                .badge-base { @apply px-2.5 py-0.5 text-xs font-semibold rounded-md inline-flex items-center gap-1 whitespace-nowrap; }
