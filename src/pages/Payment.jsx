  ...
  useEffect(() => {
    ...
    if (adultCount > currentDetailsCount) {
      const newDetails = Array.from(
        { length: adultCount - currentDetailsCount },
        () => ({ name: "", gender: "Nam", dob: "" })
      );
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white p-6 rounded-lg shadow-md sticky top-8">
              ...
              <div className="space-y-2 text-sm border-t pt-4">
                <div className="flex justify-between"><span>Người lớn</span><span>{passengers.adults} x {new Intl.NumberFormat("vi-VN").format(tour.priceAdult || 0)} ₫</span></div>
                {passengers.children > 0 && <div className="flex justify-between"><span>Trẻ em</span><span>{passengers.children} x {new Intl.NumberFormat("vi-VN").format(tour.priceChild || 0)} ₫</span></div>}
                {passengers.infants > 0 && <div className="flex justify-between"><span>Em bé</span><span>{passengers.infants} x {new Intl.NumberFormat("vi-VN").format(tour.priceInfant || 0)} ₫</span></div>}
