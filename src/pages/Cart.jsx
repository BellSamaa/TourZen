...
export default function CartPage() { // Hoặc Cart
  ...
  const handleQtyChange = (key, type, delta) => {
      ...
      let newAdults = Number(item.adults) || 0; // Đảm bảo là số
      ...
      // Bắt buộc có ít nhất 1 khách hàng
      if (newAdults === 0 && newChildren === 0 && newInfants === 0) {
    ...
    <div className="max-w-5xl mx-auto px-6 py-16">
      ...
      <div className="bg-white dark:bg-neutral-800 shadow-lg rounded-2xl overflow-hidden">
        <AnimatePresence>
            {items.map((item) => (
              <motion.div
                // Sửa: Đảm bảo item.key hoặc item.id tồn tại và là duy nhất
                key={item.key || item.id || `item-${Math.random()}`} // Fallback key nếu cần
